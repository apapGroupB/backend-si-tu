...
//    List<ValueModel> getPengajuanList();
    ...
    @Query(value = "select count(ps.id_jenis_surat), js.nama, ps.tanggal_pengajuan from pengajuan_surat ps join jenis_surat js on js.id = ps.id_jenis_surat GROUP by ps.id_jenis_surat, ps.tanggal_pengajuan;", nativeQuery = true)
